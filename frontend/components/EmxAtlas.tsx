...
interface EmxDataset {
  ...
  networks: { qcomm: { stores: number; daily_orders: number; couriers: number }; on_demand: { couriers: number } };
...
export default function EmxAtlas() {
  ...
  useEffect(() => {
    ...
    const allNetworks = () => totalHS() + REAL.networks.qcomm.daily_orders + 127;
    ...
    function showTray(label: string, r: { baseline_kpis: Record<string, { value: number }>; scenario_kpis: Record<string, { value: number }>; delta_pct: Record<string, number>; scenario_flow_feasible: boolean; delta_pct_cost?: number }) {
      ...
      $("tray").innerHTML = `<h4>${label} — engine result
        ...
        <div class="rows">${rows.map(([lab, k, u, good]) => {
          const b = r.baseline_kpis[k].value, a = r.scenario_kpis[k].value, d = r.delta_pct[k];
    ...
    $("optBtn").onclick = async () => {
      ...
      try { const r = await api("/optimize", { method: "POST", body: "{}" }, 90000);
        ...
        $("optOut").innerHTML = `
          ...
          <button class="btn ghost" style="margin-top:8px" onclick="adopt('Optimizer recommendation',${(((r.cost_to_serve_after - r.cost_to_serve_before) / r.cost_to_serve_before) * 100).toFixed(2)})">✓ Adopt</button>`;
    ...
    function renderRecs() {
      $("recs").innerHTML = `
       <div class="rec" onclick="document.querySelector('#emx-root [data-p=opt]').click()">
         ...
         <p>Fujairah ${REAL.cps.HUB_FUJ_01.toFixed(2)} & RAK ${REAL.cps.HUB_RAK_01.toFixed(2)} AED/shipment vs ${REAL.cps.HUB_DXB_01.toFixed(2)} in Dubai — run the optimizer to close the gap.</p></div>
       <div class="rec" onclick="document.querySelector('#emx-root [data-p=opt]').click()">
