...
def _overlap_model() -> NetworkModel:
    ...
    # ever competitive for Z4 -> no overlap with H1/H2.
    raw = RawTables(
        hubs=[
            {
        ...
        # All of Z1/Z2/Z3's demand parked on H1 (leaving H2 idle) so the
        # full scan_opportunities pipeline (which derives utilization from
        # THESE assignments, not a hand-fed dict) also produces a genuine
        # idle_next_to_overload finding, not just the isolated unit tests.
        current_assignments=[
...
def test_scan_opportunities_returns_all_three_types_with_consistent_totals():
    ...
    assert result["idle_next_to_overload"]  # H1 (90%+ from Z1+Z3) vs idle H2/H3
