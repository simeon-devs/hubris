...
Converts each hub's *assigned demand* (dominant-hub attribution, a real
engine output) into the courier headcount needed to move it, and
...
Every conversion factor is an explicit, named entry in the T-32
assumption registry (`core/assumptions.py`, surfaced at GET /assumptions)
and echoed in `breakdown["assumptions"]` so the UI (and any agent) can
show its working. Change a factor and every number downstream moves with
it — there is no second, hidden copy of these constants anywhere.
"""
...
# Volume basis — deliberately ASSIGNED demand, not the capacity-capped flow
# (T-37 note): the staffing question is "what does the demand pointed at
# this hub need?", and demand exceeding what the roster can sustain IS the
# understaffed signal. On the flow basis that signal is structurally
# impossible (flow <= capacity, and sustainable headcount derives from the
# same capacity), so the two bases answer different questions; utilisation
# next to this stays flow-based and the breakdown names this basis.

# ---- planning assumptions (T-32: values live in the registry) ---------------
PARCELS_PER_COURIER_HOUR = assumptions.value("parcels_per_courier_hour")
PRODUCTIVE_HOURS_PER_SHIFT = assumptions.value("productive_hours_per_shift")
PERMANENT_SHARE = assumptions.value("workforce_permanent_share")
PERMANENT_LEAD_TIME_DAYS = assumptions.value("permanent_hire_lead_time_days")
OUTSOURCED_LEAD_TIME_DAYS = assumptions.value("outsourced_hire_lead_time_days")
BALANCED_TOLERANCE_COURIERS = assumptions.value("workforce_balanced_tolerance_couriers")
    ...
    def compute(self, model: NetworkModel, scenario_id: str | None = None) -> MetricResult:
        ...
        return MetricResult(
            ...
            breakdown={
                ...
                "volume_basis": (
                    "assigned_demand (dominant-hub attribution, capacity-blind"
                    " — over-assignment is exactly the understaffed signal)"
                ),
                "assumptions": {
