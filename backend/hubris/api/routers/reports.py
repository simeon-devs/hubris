...
KPI_KEYS = ["cost_to_serve", "utilization", "coverage", "spare_capacity", "demand_served"]
# Display labels: coverage is SLA REACHABILITY (capacity-blind), demand_served
# is what the capacity-constrained flow actually serves — never conflate.
KPI_LABELS = {"coverage": "coverage (SLA reachability)", "demand_served": "demand served (capacity-constrained)"}
...
def export_report(scenario_id: str | None = None, include_optimizer: bool = False) -> Response:
    ...
    if scenario_id is not None:
        ...
        for key in KPI_KEYS:
            ...
            summary.append(
                [KPI_LABELS.get(key, key), base_v, scen_v, delta_pct, kpis.get(key, {}).get("unit", "")]
            )
    else:
        ...
        for key in KPI_KEYS:
            summary.append(
                [KPI_LABELS.get(key, key), _kpi_value(kpis, key), kpis.get(key, {}).get("unit", "")]
            )
    _autosize(summary)
