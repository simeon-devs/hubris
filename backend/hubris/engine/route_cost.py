...
Definition note (dataset G calibration): `modes[].cost_per_parcel` is a
FLEET-MODE COMPARISON figure (full trip cost amortised over one vehicle's
capacity). The network's canonical per-parcel corridor figure remains
`od_cost_per_parcel` (the calibrated od.cost the optimiser/flow prices) —
both are returned, labelled; they answer different questions and will not
generally match.
"""
